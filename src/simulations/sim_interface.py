...
class RoboticsEnvironment():
    ...
    def ActionPlace(self,placePose,approachIkTr):
        ...
        if n_configs>0:
            ...
            return 0
        pass
    def action_go_to(self,pose):
        configs = self.findConfigs(pose)
        if len(configs)>0:
            tgtConfig,passiveVizshape = self.selectOneValidConfig(configs,[],[])
            if tgtConfig is None:
                path=None
                print('Failed to find a valid configuration for desired pose')
                return 0
            self.sim.step()
            print(f'selected configuration: {tgtConfig}')
            path = self.findPath(tgtConfig)
            if passiveVizshape:
                self.sim.removeObjects([passiveVizshape])
            if path:
                self.followPath(path)
