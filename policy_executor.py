...
if __name__ == "__main__":
    ...
    parser.add_argument('-i','--perm-index', type=int, required=True, help='Permutation index of the initial state')
    parser.add_argument('-d','--training-data-collection-name', type=str, required=True, help='Name of the training data collection')
    args = parser.parse_args()

    print(f"Executing policy for {args.training_data_collection_name} with limit {args.limit} and permutation {args.perm_index} on port {args.port}")

    # Construct policy filename
    policy_filename = f'policies/{args.training_data_collection_name}_{args.limit}_points_perm_{args.perm_index}.yaml'
    ...
    for run in range(SAMPLE_COUNT):
        ...
        with open(results_filename, mode='a') as results_file:
            ...
            if not file_exists:
                ...
                file_exists = True
            init_perm = f'permutation_{args.perm_index}'
    ...
    print(f"Finished all runs for permutation {args.perm_index}. Results saved to {results_filename}")
