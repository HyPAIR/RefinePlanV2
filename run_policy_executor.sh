...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -i|--perm-index)
      ...
      shift 2
      ;;
    -d|--training-data-collection-name)
      TRAINING_DATA_COLLECTION_NAME="$2"
      shift 2
      ;;
    -h|--help)
      echo "Usage: ./run_policy_executor.sh [-p|--port PORT] [-l|--limit LIMIT] [-i|--perm-index PERM_INDEX] [-d|--training-data-collection-name TRAINING_DATA_COLLECTION_NAME]"
      exit 0
      ;;
    *)
...
# 2. Setup Variables
SESSION="policy_executor_study_${PORT}"
RUN_DIR="runs"
mkdir -p "$RUN_DIR"
...
PY_CMD="conda activate rpenv && python3 -u policy_executor.py -p $PORT -l $LIMIT -i $PERM_INDEX -d $TRAINING_DATA_COLLECTION_NAME"

echo "Initializing Session: $SESSION on Port: $PORT"
echo "Limit: $LIMIT, Permutation Index: $PERM_INDEX, Collection: $TRAINING_DATA_COLLECTION_NAME"
