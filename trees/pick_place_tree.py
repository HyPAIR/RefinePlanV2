...
def create_behaviour_tree(scene_state, goal_objects, obstacle_objects,shop_slots, goal_slots,):
    root = py_trees.composites.Sequence(name="PickPlaceRoot",memory=True)
    ...
    inverted_goal = py_trees.decorators.Inverter(name='Goal Inverter',child=goal_check)
    root.add_child(inverted_goal)
    ...
    for i, obs in enumerate(obstacle_objects):
        ...
        check = IsObstacleOnGoal(f"{obs} on goal?",scene_state,obs) # should give success if no obstalce on goal
        ...
        #Remove sequences must be inverted
        outcome_inverter = py_trees.decorators.Inverter(name='Outcome_inverter',child = sequence)
        root.add_child(outcome_inverter)
