...
function buildPassiveVizShape(robotBase)
    ...
    for i=1,#objectList do
        if sim.getBoolProperty(objectList[i], 'visible') then
    ...
    local meshHandles = sim.getIntArrayProperty(passiveVizShape, 'meshes')
    if meshHandles and #meshHandles>0 then
        sim.setColorProperty(meshHandles[1], 'color.diffuse', {1,0,0})
    end
    ...
    while i <= #configs do
        ...
        if collides({target}, robotCollection, joints) then
            ...
            if approachIKTr then
                ...
                targetPose = sim.multiplyPoses(pose, approachIKTr)
