...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -d|--training-data-collection-name)
      ...
      ;;
    -h|--help)
      echo "Usage: ./run_all_permutations.sh [-p|--port PORT] [-l|--limit LIMIT] [-d|--training-data-collection-name TRAINING_DATA_COLLECTION_NAME]"
      exit 0
      ;;
    *)
...
PY_CMD="conda activate rpenv && python3 -u policy_executor.py -p $PORT -l $LIMIT -d $TRAINING_DATA_COLLECTION_NAME"

echo "Initializing Session: $SESSION on Port: $PORT"
echo "Limit: $LIMIT, Collection: $TRAINING_DATA_COLLECTION_NAME"
