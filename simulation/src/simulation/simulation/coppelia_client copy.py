...
class RoboticEnvironment():
    ...
    def setup_ik(self):
        ...
        self.base_handle = self.sim.getObject('/UR10/link1_visible')
        print('base handle recieved')
        self.ee_handle = self.sim.getObject('/UR10/tip')
        print('end effector handle recieved')
        self.target_dummy =self.sim.getObject('/Dummy')
        constraints =0
        ...
        try:
            ikElement, simToIkObjectMap, ikToSimObjectMap=self.simIK.addElementFromScene(self.ik_env,self.ik_group,self.base_handle,self.ee_handle,self.target_dummy,constraints)
